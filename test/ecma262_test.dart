...
void main() {
  test('identifiers, whitespace, comments', () {
    ...
    expect(tokenize(file, InputElementType.div, errors), isA<NonToken>());
    expectIdentifier(tokenize(file, InputElementType.div, errors), false, 'i');
    expect(tokenize(file, InputElementType.div, errors), isA<NonToken>());
    expectIdentifier(tokenize(file, InputElementType.div, errors), false, 'am');
    expect(tokenize(file, InputElementType.div, errors), isA<NonToken>());
    expectIdentifier(tokenize(file, InputElementType.div, errors), false, 'a');
    expect(tokenize(file, InputElementType.div, errors), isA<NonToken>());
    expectIdentifier(
      tokenize(file, InputElementType.div, errors),
      false,
  ...
  test('private identifiers', () {
    SourceTextIterator file = SourceTextIterator('test', '#meow#meow says#cow');
    List<SyntaxError> errors = [];
    ...
    expectIdentifier(tokenize(file, InputElementType.div, errors), true, 'cow');
    expect(errors, isEmpty);
  });
  test('punctuators', () {
    SourceTextIterator file = SourceTextIterator(
    ...
    List<SyntaxError> errors = [];
    expectPunctuator(tokenize(file, InputElementType.div, errors), .leftBrace);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .leftParen);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .rightParen);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .leftSquare);
    expectPunctuator(
      tokenize(file, InputElementType.div, errors),
      .rightSquare,
    );
    expectPunctuator(tokenize(file, InputElementType.div, errors), .colon);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .semicolon);
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .lessEquals);
    expectPunctuator(
      tokenize(file, InputElementType.div, errors),
      .greaterEquals,
    );
    expectPunctuator(tokenize(file, InputElementType.div, errors), .leftShift);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .rightShift);
    expect(tokenize(file, InputElementType.div, errors), isA<NonToken>());
    expectPunctuator(
      tokenize(file, InputElementType.div, errors),
      .rightShiftUnsigned,
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .bitwiseNot);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .not);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .notEquals);
    expectPunctuator(
      tokenize(file, InputElementType.div, errors),
      .notEqualsEquals,
    );
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .logicalAnd);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .logicalOr);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .andEquals);
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .xorEquals);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .mod);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .modEquals);
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .powEquals);
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .addEquals);
    expectPunctuator(
      tokenize(file, InputElementType.div, errors),
      .subtractEquals,
    );
  ...
  test('numbers', () {
    ...
    expectPunctuator(tokenize(file, InputElementType.div, errors), .comma);
    expectBigInt(tokenize(file, InputElementType.div, errors), BigInt.from(1));
    expectPunctuator(tokenize(file, InputElementType.div, errors), .comma);
    expectNumber(tokenize(file, InputElementType.div, errors), 0xa);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .comma);
    expectNumber(tokenize(file, InputElementType.div, errors), 3);
    expectPunctuator(tokenize(file, InputElementType.div, errors), .comma);
    expectBigInt(tokenize(file, InputElementType.div, errors), BigInt.from(24));
